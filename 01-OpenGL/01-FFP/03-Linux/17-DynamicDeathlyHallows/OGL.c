...
void update(void)
{
	...
	if (state < 6)
	{
		angleCloakOfInvisibility = angleCloakOfInvisibility + 4.0f;
	...
	else
	{
		...
		if (diffAngleCloakOfInvisibility > 0.001f)
		{
			if (angleCloakOfInvisibility < 360.0f)
				angleCloakOfInvisibility = angleCloakOfInvisibility + (diffAngleCloakOfInvisibility / 50.0f);
			else
				angleCloakOfInvisibility = angleCloakOfInvisibility - (diffAngleCloakOfInvisibility / 50.0f);
		}
		...
		if (diffAngleResurrectionStone > 0.001f)
		{
			if (angleResurrectionStone < 360.0f)
				angleResurrectionStone = angleResurrectionStone + (diffAngleResurrectionStone / 50.0f);
			else
				angleResurrectionStone = angleResurrectionStone - (diffAngleResurrectionStone / 50.0f);
		}
	}

	if (countRotationsCloakOfInvisibility < 2)
	{
		state = 0;
	}
	else if(countRotationsCloakOfInvisibility < 8)
	{
		xCloakOfInvisibility = xCloakOfInvisibility + 0.003f;
		yCloakOfInvisibility = yCloakOfInvisibility + 0.001f;
	...
	else if (countRotationsResurrectionStone < 8)
	{
		xResurrectionStone = xResurrectionStone - 0.003f;
		yResurrectionStone = yResurrectionStone + 0.001f;
	...
	else if (countRotationsResurrectionStone < 22)
	{
		yElderWand = yElderWand - 0.003f;
