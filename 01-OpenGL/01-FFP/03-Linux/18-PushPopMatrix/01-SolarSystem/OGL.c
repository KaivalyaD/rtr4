...
/* for rendering */
float earthYear = 0.0f, earthDay = 0.0f;
float moonYear = 0.0f, moonDay = 0.0f;
const float incDec = 3.0f;
    ...
    while(!done)
    {
        ...
        while(XPending(display))
        {
            ...
            case KeyPress:
                ...
                case 'y':
                    earthYear = earthYear + incDec;
                    if(earthYear - 360.0f >= 0.0f)
                        earthYear = earthYear - 360.0f;
                    break;

                case 'Y':
                    earthYear = earthYear - incDec;
                    if(earthYear <= 0.0f)
                        earthYear = 360.0f + earthYear;
                    break;

                case 'd':
                    earthDay = earthDay + incDec;
                    if(earthDay - 360.0f >= 0.0f)
                        earthDay = earthDay - 360.0f;
                    break;

                case 'D':
                    earthDay = earthDay - incDec;
                    if(earthDay <= 0.0f)
                        earthDay = 360.0f + earthDay;
                    break;

                case 'm':
                    moonYear = moonYear + incDec;
                    if(moonYear - 360.0f >= 0.0f)
                        moonYear = moonYear - 360.0f;
                    break;

                case 'M':
                    moonYear = moonYear - incDec;
                    if(moonYear  <= 0.0f)
                        moonYear = 360.0f + moonYear;
                    break;

                case 'n':
                    moonDay = moonDay + incDec;
                    if(moonDay - 360.0f >= 0.0f)
                        moonDay = moonDay - 360.0f;
                    break;

                case 'N':
                    moonDay = moonDay - incDec;
                    if(moonDay <= 0.0f)
                        moonDay = 360.0f + moonDay;
                    break;
...
int initialize(void)
{
    ...
    // setting polygon mode to wireframe
...
void render(void)
{
    ...
    glPushMatrix();
    {
        glRotatef(earthYear, 0.0f, 1.0f, 0.0f);
        glTranslatef(1.5f, 0.0f, 0.0f);
        glPushMatrix();
        {
            glRotatef(90.0f, 1.0f, 0.0f, 0.0f);
            glRotatef(earthDay, 0.0f, 0.0f, 1.0f);
        ...
        /* drawing the moon */
        glRotatef(moonYear, 0.0f, 1.0f, 0.0f);
        glTranslatef(0.45f, 0.0f, 0.0f);
        glRotatef(moonDay, 0.0f, 1.0f, 0.0f);
        glRotatef(90.0f, 1.0f, 0.0f, 0.0f);
