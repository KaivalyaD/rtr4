...
GLubyte ubyteCheckerboard[CHECKERBOARD_WIDTH][CHECKERBOARD_HEIGHT][4];
GLuint iTextureCheckerboard;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("Procedural Textures: Kaivalya Vishwakumar Deshpande"),
...
void display(void)
{
	...
	glBindTexture(GL_TEXTURE_2D, iTextureCheckerboard);
	{
		glBegin(GL_QUADS);
		{
...
void uninitialize(void)
{
	...
	if (iTextureCheckerboard)
	{
		glDeleteTextures(1, &iTextureCheckerboard);
		iTextureCheckerboard = 0;
	}
...
void LoadGLTexture(void)
{
	...
	glGenTextures(1, &iTextureCheckerboard);
	glBindTexture(GL_TEXTURE_2D, iTextureCheckerboard);
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, CHECKERBOARD_WIDTH,
			CHECKERBOARD_HEIGHT, 0, GL_RGBA, GL_UNSIGNED_BYTE, ubyteCheckerboard);
		
		// to set floating point environment params, but not a necessary step because
		// FFP is internally implemented through PP
...
void PrepareCheckerboard(void)
{
	...
	for (int i = 0; i < CHECKERBOARD_HEIGHT; i++)
	{
		for (int j = 0; j < CHECKERBOARD_WIDTH; j++)
		{
			...
			ubyteCheckerboard[i][j][0] = (GLubyte)c;
			ubyteCheckerboard[i][j][1] = (GLubyte)c;
			ubyteCheckerboard[i][j][2] = (GLubyte)c;
			ubyteCheckerboard[i][j][3] = (GLubyte)0xff;
		}
	}
}
