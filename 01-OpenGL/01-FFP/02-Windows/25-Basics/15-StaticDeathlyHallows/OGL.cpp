...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("The Symbol of Deathly Hallows: Kaivalya Deshpande"),
...
{
	// variable declarations
	const float sideCloakOfInvisibility = 2.0f;
	...
	// drawing the Cloak of Invisibility
	glBegin(GL_LINE_LOOP);
	{
		glVertex2f(0.0f, sideCloakOfInvisibility * c);
		glVertex2f(-1 * sideCloakOfInvisibility / 2.0f, -1 * sideCloakOfInvisibility * c);
		glVertex2f(sideCloakOfInvisibility / 2.0f, -1 * sideCloakOfInvisibility * c);
	}
	glEnd();
	
	// drawing the Resurrection Stone
	glBegin(GL_LINE_LOOP);
	{
		for (int i = 0; i < 360; i++)
		{
			glVertex2f(radiusResurrectionStone * cosf(DEG_TO_RAD(i)), (radiusResurrectionStone * sinf(DEG_TO_RAD(i))) - (sideCloakOfInvisibility * c / 3.0f));
		}
	}
	glEnd();

	// drawing the Elder Wand
	glBegin(GL_LINES);
	{
		glVertex2f(0.0f, sideCloakOfInvisibility * c);
		glVertex2f(0.0f, -1 * sideCloakOfInvisibility * c);
		glVertex2f(0.001f, sideCloakOfInvisibility * c);
		glVertex2f(0.001f, -1 * sideCloakOfInvisibility * c);
	}
	glEnd();
