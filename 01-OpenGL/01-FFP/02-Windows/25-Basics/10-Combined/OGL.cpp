...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("Combined: Kaivalya Deshpande"),
...
{
	// variable declarations
	const float circleRadius = 0.25f, squareSide = 0.5f, triangleHeight = 0.4f;
	float theta;
	...
	if (gbShowGrid)
	{
		...
		for (int i = 1; i <= 20; i++)
		{
			...
			{
				// horizontals
				glVertex2f(-1.0f, i * 0.05f);
				glVertex2f(1.0f, i * 0.05f);
				glVertex2f(-1.0f, -i * 0.05f);
				glVertex2f(1.0f, -i * 0.05f);

				// verticles
				glVertex2f(-i * 0.05f, 1.0f);
				glVertex2f(-i * 0.05f, -1.0f);
				glVertex2f(i * 0.05f, 1.0f);
				glVertex2f(i * 0.05f, -1.0f);
			}
	...
	if (gbShowSquare)
	{
		// drawing the square that circumscribes everything
		glLineWidth(1.0f);
		glBegin(GL_LINE_LOOP);
		{
			glColor3f(1.0f, 0.5f, 0.0f);
			glVertex2f(squareSide / 2.0f, squareSide / 2.0f);
			glVertex2f(-1 * squareSide / 2.0f, squareSide / 2.0f);
			glVertex2f(-1 * squareSide / 2.0f, -1 * squareSide / 2.0f);
			glVertex2f(squareSide / 2.0f, -1 * squareSide / 2.0f);
		}
		glEnd();
	}
	...
	if (gbShowCircle)
	{
		// inscribed circle that circumscribes the triangle
		glLineWidth(1.0f);
		glBegin(GL_LINE_LOOP);
		{
			...
			for (int i = 0; i < 360; i++)
			{
				glVertex2f(circleRadius * cosf(DEG_TO_RAD(i)), circleRadius * sinf(DEG_TO_RAD(i)));
			}
		}
		glEnd();
	}
	...
	if (gbShowTriangle)
	{
		// inscribed triangle that circumscribes nothing
		glLineWidth(1.0f);
		glBegin(GL_LINE_LOOP);
		{
			theta = acosf((triangleHeight / circleRadius) - 1.0f);

			glColor3f(1.0f, 1.0f, 0.0f);
			glVertex2f(0.0f, circleRadius);
			glVertex2f(-1 * circleRadius * sinf(theta), circleRadius - triangleHeight);
			glVertex2f(circleRadius * sinf(theta), circleRadius - triangleHeight);
		}
		glEnd();
	}
