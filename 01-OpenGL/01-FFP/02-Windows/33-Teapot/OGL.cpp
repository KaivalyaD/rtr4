...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("Utah Teapot: Kaivalya Vishwakumar Deshpande"),
	...
	{
		fprintf(gpLog, "LoadGLTexture(): failed for texture_marble\n");
		uninitialize();
	}
	else
	{
	...
	case WM_CHAR:
		...
		case 't':
			if (!gbTextureSwitch)
			{
				...
				gbTextureSwitch = TRUE;
			}
			else
			{
				glDisable(GL_TEXTURE_2D);
				gbTextureSwitch = FALSE;
			}
			break;
		default:
			break;
...
int initialize(void)
{
	...
	pfd.cBlueBits = 8;	// B
	pfd.cAlphaBits = 8;	// A
	pfd.cDepthBits = 32; // may even be 24
