...
void display(void)
{
	...
	// rotate all models ahead until the model-view matrix is popped by 90 deg about the X-axis
	...
	// offset Earth's position from center
	glTranslatef(1.5f, 0.0f, 0.0f);

	// rotate all models ahead until the model-view matrix is popped by 90 deg about the X-axis
	glRotatef(90.0f, 1.0f, 0.0f, 0.0f);

	// rotate about self - rotation/spin
	glRotatef((GLfloat)day, 0.0f, 0.0f, 1.0f);

	// front and back shown as as wireframe
	glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);

	// create another quadric
	quadric = gluNewQuadric();
