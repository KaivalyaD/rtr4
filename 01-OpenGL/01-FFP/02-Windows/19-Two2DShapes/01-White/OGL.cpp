...
void display(void)
{
	...
	glMatrixMode(GL_MODELVIEW);	// select current operating matrix as the modelview matrix
	...
	// glLoadIdentity();
	// glTranslatef(1.5f, 0.0f, -6.0f);
	glTranslatef(3.0f, 0.0f, 0.0f);	// use this method only when it's needed (as the code grows in size, it grows increasingly clumsy to read)
	glBegin(GL_QUADS);
	{
		glVertex3f(1.0f, 1.0f, 0.0f);
		glVertex3f(-1.0f, 1.0f, 0.0f);
