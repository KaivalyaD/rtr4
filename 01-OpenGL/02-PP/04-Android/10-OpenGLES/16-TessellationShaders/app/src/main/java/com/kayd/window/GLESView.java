...
public class GLESView extends GLSurfaceView implements OnDoubleTapListener, OnGestureListener, GLSurfaceView.Renderer {
    ...
    private boolean isSegmentCountIncreasing;
    ...
    public boolean onSingleTapConfirmed(MotionEvent e) {
        if(isSegmentCountIncreasing) {
            numberOfSegments += 1;
            if(numberOfSegments == 30)
                isSegmentCountIncreasing = false;
        }
        else {
            numberOfSegments -= 1;
            if(numberOfSegments == 1)
                isSegmentCountIncreasing = true;
        }
        return true;
    }
    ...
    private void initialize() {
        ...
        GLES32.glGetShaderiv(tessellationControlShaderObject, GLES32.GL_COMPILE_STATUS, status, 0);
        if(status[0] == GLES32.GL_FALSE) {
            System.out.println("KVD: *** tessellation control shader compilation errors ***");

            GLES32.glGetShaderiv(tessellationControlShaderObject, GLES32.GL_INFO_LOG_LENGTH, infoLogLength, 0);
            if(infoLogLength[0] > 0) {
                infoLog = GLES32.glGetShaderInfoLog(tessellationControlShaderObject);
                System.out.println("KVD: tessellation control shader compilation log (" + infoLogLength[0] + " bytes):");
                System.out.println(infoLog);
            }
            else {
                System.out.println("KVD: there is nothing to print");
        ...
        final String tessellationEvaluationShaderSourceCode = String.format(
            ...
            "   \n" +
            "   vec3 P = P0 * (1.0 - u) * (1.0 - u) * (1.0 - u) + " +
                    "P1 * 3.0 * u * (1.0 - u) * (1.0 - u) + " +
                    "P2 * 3.0 * u * u * (1.0 - u) + " +
                    "P3 * u * u * u;\n" +
            "   \n" +
            "   gl_Position = u_mvpMatrix * vec4(P, 1.0);\n" +
            "}\n"  /* declare u as a double if using this shader in vulkan */
        );
        ...
        GLES32.glGetShaderiv(tessellationEvaluationShaderObject, GLES32.GL_COMPILE_STATUS, status, 0);
        if(status[0] == GLES32.GL_FALSE) {
            System.out.println("KVD: *** tessellation evaluation shader compilation errors ***");

            GLES32.glGetShaderiv(tessellationEvaluationShaderObject, GLES32.GL_INFO_LOG_LENGTH, infoLogLength, 0);
            if(infoLogLength[0] > 0) {
                infoLog = GLES32.glGetShaderInfoLog(tessellationEvaluationShaderObject);
                System.out.println("KVD: tessellation evaluation shader compilation log (" + infoLogLength[0] + " bytes):");
                System.out.println(infoLog);
            }
            else {
                System.out.println("KVD: there is nothing to print");
        ...
        GLES32.glBindVertexArray(vao[0]);
        {
            ...
            GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vbo[0]);
            {
                ...
                GLES32.glVertexAttribPointer(GLESView.KVD_ATTRIBUTE_POSITION, 2, GLES32.GL_FLOAT, false, 0, 0);
    ...
    private void render() {
        ...
        GLES32.glUseProgram(shaderProgramObject);
        {
            ...
            GLES32.glUniform1i(numberOfSegmentsUniform, numberOfSegments);
            GLES32.glUniform1i(numberOfStripsUniform, 1);
            if(numberOfSegments == 30)
                GLES32.glUniform4f(lineColorUniform, 0.0f, 1.0f, 0.0f, 1.0f);
            else
