...
-(void) initialize
{
    ...
    textureSmiley = [self loadGLTexture: "Resources/Smiley.bmp"];
    if(!textureSmiley)
    {
        fprintf(logFile, "loadGLTexture: failed to load Smiley.bmp\n");
        [self uninitialize];
        [self release];
        [NSApp terminate: self];
    }
...
-(void) renderScene
{
    ...
    {
        // transformations
        modelViewMatrix = vmath::translate(0.0f, 0.0f, -4.5f);
