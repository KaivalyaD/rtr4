...
-(void) initialize
{
    ...
    textureStone = [self loadGLTexture: "Resources/Stone.bmp"];
    if(!textureStone)
    {
        fprintf(logFile, "loadGLTexture: failed to load Stone.bmp\n");
        [self uninitialize];
        [self release];
        [NSApp terminate: self];
    }
    
    textureKundali = [self loadGLTexture: "Resources/Vijay_Kundali.bmp"];
    if(!textureKundali)
    {
        fprintf(logFile, "loadGLTexture: failed to load Vijay_Kundali.bmp\n");
        [self uninitialize];
        [self release];
        [NSApp terminate: self];
    }
...
-(void) renderScene
{
    ...
    glUseProgram(shaderProgramObject);
    {
        ...
        modelViewMatrix = vmath::translate(-1.5f, 0.0f, -6.0f) * vmath::rotate(-anglePyramid, 0.0f, 1.0f, 0.0f);
