...
const unsigned int arraySize = meshWidth * meshHeight * 4;

float positions[meshHeight][meshWidth][4];
float animTime = 0.0f;
...
int initialize(void)
{
    ...
    void uninitialize(void);
    void resize(int, int);
    void checkCLError(const char *, int);
    ...
    // declarations for OpenCL
    cl_uint oclPlatformCount;
    ...
    /*********** OpenCL initialization begins *****************/

    // obtain platform and device at ID 0, if available
    oclErr = clGetPlatformIDs(0, NULL, &oclPlatformCount); checkCLError("clGetPlatformIDs", __LINE__);
    if(oclPlatformCount <= 0) {
        fprintf(logFile, "fatal: no OpenCL supporting platforms found\n");
        uninitialize();
        exit(EXIT_FAILURE);
    }
    oclErr = clGetPlatformIDs(1, &oclPlatformId, NULL); checkCLError("clGetPlatformIDs", __LINE__);
    
    oclErr = clGetDeviceIDs(oclPlatformId, CL_DEVICE_TYPE_GPU, 0, NULL, &oclDevCount); checkCLError("clGetDeviceIDs", __LINE__);
    if(oclDevCount <= 0) {
        fprintf(logFile, "fatal: no OpenCL supporting GPU devices found\n");
        uninitialize();
        exit(EXIT_FAILURE);
    }
    oclErr = clGetDeviceIDs(oclPlatformId, CL_DEVICE_TYPE_GPU, 1, &oclDeviceId, NULL); checkCLError("clGetDeviceIDs", __LINE__);

    // create a cl-gl context
    ...
    oclContext = clCreateContext(oclContextProperties, 1, &oclDeviceId, NULL, NULL, &oclErr); checkCLError("clCreateContext", __LINE__);

    // create a command queue
    oclCmdQueue = clCreateCommandQueueWithProperties(oclContext, oclDeviceId, NULL, &oclErr); checkCLError("clCreateCommandQueueWithProperties", __LINE__);

    // create a program
    const char *oclKernelSourceCode =
        "__kernel void animateSineWaveOnGPU(__global float4 *positions, unsigned int width, unsigned int height, float phase)\n" \
        "{\n" \
        "    float freq = 4.0f;\n" \
        ...
        "        v = (2.0f * v) - 1.0f;\n" \
        "    \n" \
        "        positions[(x * width) + z] = (float4)(u, 0.5f * sin((u * freq) + phase) * cos((v * freq) + phase), v, 1.0f);\n" \
        "    }" \
        "}\n";
    oclProgram = clCreateProgramWithSource(oclContext, 1, &oclKernelSourceCode, NULL, &oclErr); checkCLError("clCreateProgramWithSource", __LINE__);

    // build the program
    oclErr = clBuildProgram(oclProgram, 1, &oclDeviceId, NULL, NULL, NULL);
    if(oclErr != CL_SUCCESS)
    {
        const int _orig_lineNum = __LINE__ - 3;
        const int _orig_oclErr = oclErr;
        
        size_t logSize;
        char *logBuffer = NULL;

        oclErr = clGetProgramBuildInfo(oclProgram, oclDeviceId, CL_PROGRAM_BUILD_LOG, 0, NULL, &logSize); checkCLError("clGetProgramBuildInfo", __LINE__);
        logBuffer = (char *)alloca(logSize * sizeof(char));
        oclErr = clGetProgramBuildInfo(oclProgram, oclDeviceId, CL_PROGRAM_BUILD_LOG, logSize * sizeof(char), logBuffer, NULL); checkCLError("clGetProgramBuildInfo", __LINE__);

        fprintf(logFile, "fatal: compilation of 'animateSineWaveOnGPU' failed:\n");
        fprintf(logFile, "build log (%d bytes):\n", logSize);
        fprintf(logFile, "%s\n", logBuffer);

        oclErr = _orig_oclErr; checkCLError("clBuildProgram", _orig_lineNum);
    }

    // create a kernel from program
    oclKernel = clCreateKernel(oclProgram, "animateSineWaveOnGPU", &oclErr); checkCLError("clCreateKernel", __LINE__);
    ...
    /* create CL/GL interop resource */
    graphicsResource = clCreateFromGLBuffer(oclContext, CL_MEM_WRITE_ONLY, (cl_GLuint)gpuVbo, &oclErr); checkCLError("clCreateFromGLBuffer", __LINE__);
...
void render(void)
{
	...
	glUseProgram(shaderProgramObject);
    {
        glBindVertexArray(vao);
        {
            ...
            if(!onGPU)
            {
                sprintf(title, "Kaivalya Deshpande: OpenGL-OpenCL | Points Rendered = %d | Running on CPU", meshWidth * meshHeight);
            ...
            {
                sprintf(title, "Kaivalya Deshpande: OpenGL-OpenCL | Points Rendered = %d | Running on GPU", meshWidth * meshHeight);
                XStoreName(display, window, title);

                // set kernel parameters
                clSetKernelArg(oclKernel, 0, sizeof(cl_mem), (void *)&graphicsResource); checkCLError("clSetKernelArg", __LINE__);
                clSetKernelArg(oclKernel, 1, sizeof(cl_uint), (void *)&meshWidth); checkCLError("clSetKernelArg", __LINE__);
                clSetKernelArg(oclKernel, 2, sizeof(cl_uint), (void *)&meshHeight); checkCLError("clSetKernelArg", __LINE__);
                clSetKernelArg(oclKernel, 3, sizeof(cl_float), (void *)&animTime); checkCLError("clSetKernelArg", __LINE__);

                // enqueue lock on graphics resource for use by opencl
                oclErr = clEnqueueAcquireGLObjects(oclCmdQueue, 1, &graphicsResource, 0, NULL, NULL); checkCLError("clEnqueueAcquireGLObjects", __LINE__);

                // compute work size according to meshWidth and meshHeight
                globalWorkSize[0] = meshWidth;
                globalWorkSize[1] = meshHeight;

                // enqueue kernel launch
                oclErr = clEnqueueNDRangeKernel(oclCmdQueue, oclKernel, 2, NULL, globalWorkSize, NULL, 0, NULL, NULL); checkCLError("clEnqueueNDRangeKernel", __LINE__);

                // enqueue release on the lock for use by opengl
                oclErr = clEnqueueReleaseGLObjects(oclCmdQueue, 1, &graphicsResource, 0, NULL, NULL); checkCLError("clEnqueueReleaseGLObjects", __LINE__);

                // ensure GL can access the buffer
...
/* opencl error handling utility function */
void checkCLError(const char *func, int lineNum)
{
    // code
    if(oclErr != CL_SUCCESS) {
        fprintf(logFile, "error: %s() : line %d : returned with code %d\n", func, lineNum, oclErr);
        uninitialize();
        exit(EXIT_FAILURE);
    }
}
