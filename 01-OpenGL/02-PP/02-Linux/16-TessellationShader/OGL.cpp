...
int initialize(void)
{
    ...
    if (status == GL_FALSE)
    {
        fprintf(logFile, "*** tessellation control shader compilation errors ***\n");
        ...
        if (infoLogLength > 0)
        {
            ...
            if (infoLog)
            {
                ...
                glGetShaderInfoLog(tessellationControlShaderObject, infoLogLength * sizeof(char), &written, infoLog);
                fprintf(logFile, "tessellation control shader compilation log (%d bytes):\n%s\n", written, infoLog);
                free(infoLog);
                infoLog = NULL;
            }
            else
    ...
    fprintf(logFile, "tessellation control shader was compiled without errors\n");
    ...
    if (status == GL_FALSE)
    {
        fprintf(logFile, "*** tessellation evaluation shader compilation errors ***\n");
        ...
        if (infoLogLength > 0)
        {
            ...
            if (infoLog)
            {
                ...
                glGetShaderInfoLog(tessellationEvaluationShaderObject, infoLogLength * sizeof(char), &written, infoLog);
                fprintf(logFile, "tessellation evaluation shader compilation log (%d bytes):\n%s\n", written, infoLog);
                free(infoLog);
                infoLog = NULL;
            }
            else
    ...
    fprintf(logFile, "tessellation evaluation shader was compiled without errors\n");
...
void render(void)
{
	...
	glUseProgram(shaderProgramObject);
    {
        ...
        sprintf(windowTitle, "Kaivalya Vishwakumar Deshpande: OpenGL | Number of Segments = %d", numberOfSegments);
        XStoreName(display, window, windowTitle);

        // setting up uniforms
        glUniform1i(numberOfSegmentsUniform, numberOfSegments);
