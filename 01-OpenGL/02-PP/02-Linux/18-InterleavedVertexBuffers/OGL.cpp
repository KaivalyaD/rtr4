...
int initialize(void)
{
	...
	const GLchar *vertexShaderSourceCode =
        ...
        "{\n"
            ...
            "vec3 lightPosition;  // light position in eye coordinates\n" \
        "} ub_light;\n" \
        "\n" \
        ...
        "{\n" \
            ...
            "{\n" \
                ...
                "\n" \
                "vs_out.diffuseLightColor = ub_light.Ld * ub_material.Kd * max(dot(lightDirection, transformedNormal), 0.0);\n" \
            "}\n" \
            "\n" \
	...
	const GLchar *fragmentShaderSourceCode =
        ...
        "\n" \
        "layout (location = 0) uniform int u_lightEnabled;\n" \
        "layout (location = 1) uniform sampler2D u_textureSampler;\n" \
        "\n" \
        "out vec4 FragColor;\n" \
        "\n" \
        "void main(void)\n" \
        "{\n" \
            "if (u_lightEnabled == 1)\n" \
                "FragColor = fs_in.color * texture(u_textureSampler, fs_in.texCoord) * vec4(fs_in.diffuseLightColor, 1.0);\n" \
            "else\n" \
                "FragColor = fs_in.color * texture(u_textureSampler, fs_in.texCoord);\n" \
        "}\n";
    ...
    glBindVertexArray(vao);
    {
        ...
        {
            glBufferStorage(GL_ARRAY_BUFFER, 2 * sizeof(vmath::vec4), NULL, GL_DYNAMIC_STORAGE_BIT | GL_MAP_WRITE_BIT);
    ...
    // loading textures
    if(!loadGLTexture(&texture_marble, "./res/Marble.png")) {
        fprintf(logFile, "loadGLTexture(): failed to load texture_marble\n");
        uninitialize();
        exit(EXIT_FAILURE);
    }
    ...
    void *pMappedBuf;
    pMappedBuf = glMapNamedBufferRange(uboLight, 0, 2 * sizeof(vmath::vec4), GL_MAP_WRITE_BIT);
    {
        ((vmath::vec4 *)(pMappedBuf))[0] = vmath::vec4(1.0f, 1.0f, 1.0f, 1.0f);
        ((vmath::vec4 *)(pMappedBuf))[1] = vmath::vec4(0.0f, 0.0f, 2.0f, 1.0f);
    }
    glUnmapNamedBuffer(uboLight);
    pMappedBuf = NULL;

    // setting material params
    pMappedBuf = glMapNamedBufferRange(uboMaterial, 0, 1 * sizeof(vmath::vec4), GL_MAP_WRITE_BIT);
    {
        ((vmath::vec4 *)(pMappedBuf))[0] = vmath::vec4(0.5f, 0.5f, 0.5f, 1.0f);
    }
