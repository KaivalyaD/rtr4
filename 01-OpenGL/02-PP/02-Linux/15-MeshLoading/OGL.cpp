...
GLuint textureSuzanne;

vmath::mat4 perspectiveProjectionMatrix;

/* for the model */
struct mesh_t mesh_suzanne;
struct opengl_compatible_mesh_t ogl_mesh_suzanne;
...
void initialize(void)
{
    ...
    {
        fprintf(logFile, "glXGetProcAddressARB(): failed\n");
        uninitialize();
        exit(EXIT_FAILURE);
    }
    ...
    {
		fprintf(logFile, "glewInit(): failed with code %d\n", ret);
        uninitialize();
        exit(EXIT_FAILURE);
    }

    // OpenGL has initialized successfully by this time
	...
	const GLchar *vertexShaderSourceCode =
		...
		"{\n" \
            ...
            "vec4 eyeCoordinate = u_mvMatrix * a_position;\n" \
            "\n" \
            "vs_out.transformedNormal = normalize(normalMatrix * a_normal);\n" \
            "vs_out.lightDirection = normalize(u_lightPosition.xyz - eyeCoordinate.xyz);\n" \
            "vs_out.texCoord = a_texCoord;\n" \
            "vs_out.view = -eyeCoordinate.xyz;\n"\
            "\n" \
	...
	const GLchar *fragmentShaderSourceCode =
		...
		"void main(void)\n" \
		"{\n" \
            "vec3 normal = normalize(fs_in.transformedNormal);\n" \
            "vec3 lightDirection = normalize(fs_in.lightDirection);\n" \
            "\n" \
            "vec3 view = normalize(fs_in.view);\n" \
            "vec3 refl = reflect(-lightDirection, normal);\n" \
            "\n"
            "vec4 texColor = texture(u_textureSampler, fs_in.texCoord);\n"\
            "\n"
			"FragColor = texColor * u_Ld * u_Kd * (0.5 + max(dot(lightDirection, normal), 0.0) + pow(max(dot(refl, view), 0.0), 128.0));\n" \
		"}\n";
    ...
    mvMatrixUniform = glGetUniformLocation(shaderProgramObject, "u_mvMatrix");
    projMatrixUniform = glGetUniformLocation(shaderProgramObject, "u_projMatrix");
    textureSamplerUniform = glGetUniformLocation(shaderProgramObject, "u_textureSampler");

    /* loading vertex data into pipeline */
    // loading the mesh
    mesh_create(&mesh_suzanne);
    if(mesh_load_from_obj(&mesh_suzanne, "./suzanne.obj") != MESH_SUCCESS)
    {
        printf("mesh_load_from_obj(): failed to load ./suzanne.obj\n");
        uninitialize();
        exit(EXIT_FAILURE);
    }
    fprintf(
        logFile,
        "successfully loaded (%d vertices, %d faces) from meshfile: ./suzanne.obj\n",
        mesh_suzanne.vertex_count, mesh_suzanne.face_count
    );

    // get opengl compatible version of loaded mesh
    mesh_set_opengl_attrib_location_position(KVD_ATTRIBUTE_POSITION);
    mesh_set_opengl_attrib_location_normal(KVD_ATTRIBUTE_NORMAL);
    mesh_set_opengl_attrib_location_texcoord(KVD_ATTRIBUTE_TEXTURE0);
    if((ret = mesh_create_opengl_compatible_mesh(&mesh_suzanne, &ogl_mesh_suzanne)) != MESH_SUCCESS)
    {
        fprintf(logFile, "mesh_create_opengl_compatible_mesh(): failed with code %d\n", ret);
        uninitialize();
        exit(EXIT_FAILURE);
    }

    // loading textures
    if(!loadGLTexture(&textureSuzanne, "./res/suzanne.png"))
    {
        fprintf(logFile, "loadGLTexture(): failed to load suzanne.png\n");
        uninitialize();
        exit(EXIT_FAILURE);
    }
...
void render(void)
{
	...
	glUseProgram(shaderProgramObject);
    {
        ...
        glUniformMatrix4fv(mvMatrixUniform, 1, GL_FALSE, vmath::translate(0.0f, 0.0f, -4.0f) * vmath::rotate(theta, 0.0f, 1.0f, 0.0f));
        glUniformMatrix4fv(projMatrixUniform, 1, GL_FALSE, perspectiveProjectionMatrix);

        // lighting uniforms
        glUniform4f(ldUniform, 1.0f, 1.0f, 1.0f, 1.0f);
        glUniform4f(kdUniform, 0.6f, 0.3f, 0.2f, 1.0f);
        glUniform4f(lightPositionUniform, 0.0f, 5.0f, 10.0f, 1.0f);
        ...
        glBindVertexArray(ogl_mesh_suzanne.vao);
            glDrawArrays(GL_TRIANGLES, 0, mesh_suzanne.face_count * 3);
        glBindVertexArray(0);
...
{
    // function prototypes
    void mesh_destroy_opengl_compatible_mesh(struct opengl_compatible_mesh_t *);
    void mesh_destroy(struct mesh_t *);
    ...
    /* free allocated opengl compatible mesh */
    mesh_destroy_opengl_compatible_mesh(&ogl_mesh_suzanne);

    /* free allocated mesh */
    mesh_destroy(&mesh_suzanne);
