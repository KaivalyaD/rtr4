...
void render(void)
{
    ...
    glUseProgram(shaderProgramObject);
    {
        ...
        push(modelViewMatrix, stackMatModelView);
        {
            ...
            modelViewMatrix = top(stackMatModelView) *
            ...
            push(modelViewMatrix, stackMatModelView);
            {
                glUniformMatrix4fv(modelViewMatrixUniform, 1, GL_FALSE, top(stackMatModelView) * vmath::scale(0.2f));
                glBindVertexArray(vaoSphere);
                    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vboSphereElements);
                    glDrawElements(GL_TRIANGLES, numElements, GL_UNSIGNED_SHORT, 0);
                
                glUniformMatrix4fv(modelViewMatrixUniform, 1, GL_FALSE, top(stackMatModelView));
        ...
        pop(stackMatModelView);  // pop initial transformation
    }
    glUseProgram(0);
