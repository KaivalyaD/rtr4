...
int initialize(void)
{
	...
	{
        fprintf(logFile, "loadGLTexture(): failed for stone.png\n");
        uninitialize();
        exit(EXIT_FAILURE);
    }
	if (!loadGLTexture(&textureKundali, "./resources/vijay_kundali.png"))
	{
        fprintf(logFile, "loadGLTexture(): failed for vijay_kundali.png\n");
        uninitialize();
        exit(EXIT_FAILURE);
    }
