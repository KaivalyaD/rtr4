/*
 * Mesh Loader v1.0 Implementation
...
int mesh_load_from_obj(struct mesh_t *mesh, const char *meshfile)
{
    ...
    // code
    if(stat(meshfile, &meshFileInfo) == 0)
        meshFileSize = meshFileInfo.st_size;
    else
    {
        return MESH_ERR_UNKNOWNFILESIZE;
    }

    buf_copy = buf = (char *)malloc(sizeof(char) * meshFileSize);
    if(!buf)
    {
    ...
    {
        // reading mesh file
        if(fread(buf_copy, sizeof(char), meshFileSize, meshFile) != meshFileSize)
        {
            return MESH_ERR_FILEUNREAD;
        }

        line = strtok_r(buf_copy, "\n", &line_save_ptr);
        while(line != NULL)
        {
            switch(line[0])
            {
