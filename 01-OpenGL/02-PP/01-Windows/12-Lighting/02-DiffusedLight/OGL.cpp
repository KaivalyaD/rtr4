...
int initialize(void)
{
	...
	const GLchar *vertexShaderSourceCode =
		...
		"{\n" \
			...
			"{\n" \
				...
				"\n" \
				"diffuseLightColor = u_Ld * u_Kd * max(dot(lightDirection, transformedNormal), 0.0);\n" \
			"}\n" \
			"\n" \
	...
	enableLightsUniform = glGetUniformLocation(shaderProgramObject, "u_enableLights");
	
	// vao
	glGenVertexArrays(1, &vao);
	glBindVertexArray(vao);
	{
		// position vboPositions
...
void display(void)
{
	...
	{
		// transformations
		rotationMatrix_X = vmath::rotate(thetaZYX, 1.0f, 0.0f, 0.0f);
		rotationMatrix_Y = vmath::rotate(thetaZYX, 0.0f, 1.0f, 0.0f);
		rotationMatrix_Z = vmath::rotate(thetaZYX, 0.0f, 0.0f, 1.0f);
		...
		if (enableLights)
		{
			glUniform1i(enableLightsUniform, 1);
		...
		{
			glUniform1i(enableLightsUniform, 0);
		}
...
{
	// code
	thetaZYX = thetaZYX + 1.0f;
	if (thetaZYX >= 360.0f)
		thetaZYX = thetaZYX - 360.0f;
}
