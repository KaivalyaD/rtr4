...
int initialize(void)
{
	...
	const GLchar *geometryShaderSourceCode =
		"#version 460 core\n" \
		"\n" \
		"layout(triangles) in;  // domain of this geometry shader: triangles\n" \
		"\n" \
		"uniform mat4 u_mvpMatrix;\n" \
		"\n" \
	...
	if (status == GL_FALSE)
	{
		fprintf(gpLog, "*** geometry shader compilation errors ***\n");
		...
		if (infoLogLength > 0)
		{
			...
			if (log)
			{
				...
				glGetShaderInfoLog(geometryShaderObject, infoLogLength * sizeof(char), &written, log);
				fprintf(gpLog, "geometry shader compilation log (%d bytes):\n%s\n", written, log);
				free(log);
				log = NULL;
			}
			else
	...
	fprintf(gpLog, "geometry shader was compiled without errors\n");
