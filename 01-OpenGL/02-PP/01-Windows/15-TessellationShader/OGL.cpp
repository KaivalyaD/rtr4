...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_KEYDOWN:
        ...
        case VK_UP:
            uiNumberOfSegments++;
            if (uiNumberOfSegments >= MAX_SEGMENTS)
                uiNumberOfSegments = MAX_SEGMENTS;
            break;
        case VK_DOWN:
            uiNumberOfSegments--;
...
int initialize(void)
{
    ...
    if (status == GL_FALSE)
    {
        fprintf(gpLog, "*** tessellation control shader compilation errors ***\n");
        ...
        if (infoLogLength > 0)
        {
            ...
            if (log)
            {
                ...
                glGetShaderInfoLog(tessellationControlShaderObject, infoLogLength * sizeof(char), &written, log);
                fprintf(gpLog, "tessellation control shader compilation log (%d bytes):\n%s\n", written, log);
                free(log);
                log = NULL;
            }
            else
    ...
    fprintf(gpLog, "tessellation control shader was compiled without errors\n");
    ...
    if (status == GL_FALSE)
    {
        fprintf(gpLog, "*** tessellation evaluation shader compilation errors ***\n");
        ...
        if (infoLogLength > 0)
        {
            ...
            if (log)
            {
                ...
                glGetShaderInfoLog(tessellationEvaluationShaderObject, infoLogLength * sizeof(char), &written, log);
                fprintf(gpLog, "tessellation evaluation shader compilation log (%d bytes):\n%s\n", written, log);
                free(log);
                log = NULL;
            }
            else
    ...
    fprintf(gpLog, "tessellation evaluation shader was compiled without errors\n");
    ...
    uiNumberOfSegments = 1;  // drawing atleast 1 segment in the curve
...
void display(void)
{
    ...
    glUniformMatrix4fv(mvpMatrixUniform, 1, GL_FALSE, modelViewProjectionMatrix);
    {
        ...
        TCHAR str[255];
        wsprintf(str, TEXT("\nKaivalya Vishwakumar Deshpande: OpenGL | Number of Segments = %d"), uiNumberOfSegments);
        SetWindowText(ghwnd, str);
        
        glUniform1i(numberOfStripsUniform, 1);  // a control similar to GL_LINE_STRIP
