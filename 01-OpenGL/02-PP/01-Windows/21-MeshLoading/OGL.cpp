...
int initialize(void)
{
	...
	if ((ret = mesh_load_from_obj(&mesh_suzanne, "./suzanne.obj")) != MESH_SUCCESS)
	{
		fprintf(gpLog, "mesh_load_from_obj(): returned with code %d\n", ret);
		uninitialize();
		exit(EXIT_FAILURE);
	}
	fprintf(
	...
	{
		fprintf(gpLog, "mesh_create_opengl_compatible_mesh(): failed with code %d\n", ret);
		uninitialize();
		exit(EXIT_FAILURE);
	}

	// loading textures
	if (!LoadGLTexture(&textureSuzanne, MAKEINTRESOURCE(IDBITMAP_SUZANNE)))
	{
		fprintf(gpLog, "LoadGLTexture(): failed to load Suzanne.bmp\n");
		uninitialize();
		exit(EXIT_FAILURE);
	}
