...
int initialize(void)
{
	...
	glBindVertexArray(vaoTriangle);
	{
		...
		{
			const GLfloat triangleVertexPositions[] = {
				 0.0f,  1.0f, 0.0f,
				-1.0f, -1.0f, 0.0f,
				 1.0f, -1.0f, 0.0f
			};

			glBufferData(GL_ARRAY_BUFFER, sizeof(triangleVertexPositions), triangleVertexPositions, GL_STATIC_DRAW);
		...
		{
			const GLfloat triangleVertexColors[] = {
				1.0f, 0.0f, 0.0f,  // R
				0.0f, 1.0f, 0.0f,  // G
				0.0f, 0.0f, 1.0f   // B
			};
			
			glBufferData(GL_ARRAY_BUFFER, sizeof(triangleVertexColors), triangleVertexColors, GL_STATIC_DRAW);
	...
	glBindVertexArray(vaoSquare);
	{
		...
		{
			const GLfloat squareVertexPositions[] = {
				1.0f,  1.0f, 0.0f,
			   -1.0f,  1.0f, 0.0f,
			   -1.0f, -1.0f, 0.0f,
				1.0f, -1.0f, 0.0f
			...
			glBufferData(GL_ARRAY_BUFFER, sizeof(squareVertexPositions), squareVertexPositions, GL_STATIC_DRAW);
			glVertexAttribPointer(KVD_ATTRIBUTE_POSITION, 3, GL_FLOAT, GL_FALSE, 0, NULL);
			glEnableVertexAttribArray(KVD_ATTRIBUTE_POSITION);
		}
		glBindBuffer(GL_ARRAY_BUFFER, 0);

		glVertexAttrib3f(KVD_ATTRIBUTE_COLOR, 0.0f, 0.0f, 1.0f);
	}
	glBindVertexArray(0);
...
void display(void)
{
	...
	glUseProgram(shaderProgramObject);
	{
		...
		modelViewMatrix = translationMatrix * rotationMatrix;  // rotate after translation
