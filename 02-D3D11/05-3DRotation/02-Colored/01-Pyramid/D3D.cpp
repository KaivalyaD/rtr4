...
HRESULT initialize(void)
{
	...
	d3d11InputElementDescriptor[1].InputSlot = KVD_ATTRIBUTE_COLOR;
	...
	d3d11BufferDescriptor.ByteWidth = sizeof(float) * _ARRAYSIZE(vertexColors);
	d3d11BufferDescriptor.BindFlags = D3D11_BIND_VERTEX_BUFFER;

	// populate
	ZeroMemory((void *)&d3d11SubresourceData, sizeof(d3d11SubresourceData));
	d3d11SubresourceData.pSysMem = vertexColors;

	// create
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDescriptor, &d3d11SubresourceData, &gpID3D11Buffer_ColorBuffer);
	if (FAILED(hr))
	{
		fopen_s(&gpLog, "Log.txt", "a+");
		fprintf(gpLog, "initialize: CreateBuffer for Color Buffer failed with error code 0x%lx\n", hr);
		fclose(gpLog);
	}
	else
	{
		fopen_s(&gpLog, "Log.txt", "a+");
		fprintf(gpLog, "initialize: CreateBuffer for Color Buffer succeeded\n");
		fclose(gpLog);
	}
...
void display(void)
{
	...
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(KVD_ATTRIBUTE_COLOR, 1, &gpID3D11Buffer_ColorBuffer, &stride, &offset);

	// set primitive topology in input assembly stage of pipeline
	gpID3D11DeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

	// transformations
	XMMATRIX worldMatrix = XMMatrixRotationY(XMConvertToRadians(theta)) * XMMatrixTranslation(0.0f, 0.0f, 4.0f);
