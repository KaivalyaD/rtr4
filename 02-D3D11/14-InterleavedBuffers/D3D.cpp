...
HRESULT initialize(void)
{
	...
	const char *vertexShaderSourceCode =
		...
		"	float4 color: VSOUT_COLOR;\n" \
		"	float2 texCoord: VSOUT_TEXCOORD;\n" \
		"};\n" \
		"\n" \
	...
	const char *pixelShaderSourceCode =
		...
		"		\n" \
		"		return input.color * myTexture2D.Sample(mySamplerState, input.texCoord) * (ambient + diffuse + specular);\n" \
		"	}\n" \
		"	return input.color * myTexture2D.Sample(mySamplerState, input.texCoord);\n" \
		"}\n";
	...
	d3d11InputElementDescriptor[2].Format = DXGI_FORMAT_R32G32B32_FLOAT;  // pass as a float3
	...
	d3d11InputElementDescriptor[3].Format = DXGI_FORMAT_R32G32_FLOAT;  // pass as a float2
	...
	d3d11BufferDescriptor.ByteWidth = sizeof(float) * _ARRAYSIZE(vertexData);
	...
	d3d11SubresourceData.pSysMem = vertexData;

	// create
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDescriptor, &d3d11SubresourceData, &gpID3D11Buffer_VertexBuffer);
	if (FAILED(hr))
	{
		fopen_s(&gpLog, "Log.txt", "a+");
		fprintf(gpLog, "initialize: CreateBuffer for Vertex Buffer failed with error code 0x%lx\n", hr);
		fclose(gpLog);
	}
	else
	{
		fopen_s(&gpLog, "Log.txt", "a+");
		fprintf(gpLog, "initialize: CreateBuffer for Vertex Buffer succeeded\n");
		fclose(gpLog);
	}
	...
	// create texture
	hr = LoadD3DTexture(L"./resources/Marble.bmp", &gpID3D11ShaderResourceView_Texture);
	if (FAILED(hr))
	{
		fopen_s(&gpLog, "Log.txt", "a+");
		fprintf(gpLog, "initialize: LoadD3DTexture failed for Marble.bmp with error code 0x%lx\n", hr);
		fclose(gpLog);
		return hr;
	}
	else
	{
		fopen_s(&gpLog, "Log.txt", "a+");
		fprintf(gpLog, "initialize: LoadD3DTexture succeeded for Marble.bmp\n");
		fclose(gpLog);
	}
...
void display(void)
{
	...
	// set texcoord buffer in input assembly stage of pipeline
	stride = sizeof(float) * 11;
	...
	// transformations
	XMMATRIX worldMatrix = XMMatrixRotationX(XMConvertToRadians(theta)) *
