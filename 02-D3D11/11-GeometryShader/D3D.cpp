...
HRESULT initialize(void)
{
	...
	const char *geometryShaderSourceCode =
		...
		"	for(int i = 0; i < 3; i++)\n" \
		"	{\n" \
		"		output.position = mul(worldViewProjectionMatrix, input[i].position + float4(0.0f, 1.0f, 0.0f, 1.0f));\n" \
		"		outputStream.Append(output);\n" \
		"		\n" \
		"		output.position = mul(worldViewProjectionMatrix, input[i].position + float4(1.0f, -1.0f, 0.0f, 1.0f));\n" \
		"		outputStream.Append(output);\n" \
		"		\n" \
		"		output.position = mul(worldViewProjectionMatrix, input[i].position + float4(-1.0f, -1.0f, 0.0f, 1.0f));\n" \
		"		outputStream.Append(output);\n" \
	...
	hr = D3DCompile(
		...
		D3D_COMPILE_STANDARD_FILE_INCLUDE,
		"main",
		"gs_5_0",  // geometry shader feature level 5.0
		0,  // compile constants, like fast-relaxed-math in OpenCL
		0,	// effect constants, when using fxc.exe
		&pID3DBlob_GeometryShaderCode,  // compiled code blob
		&pID3DBlob_Error  // error blob
	);
	if (FAILED(hr))
	{
		...
		if (pID3DBlob_Error)
		{
			fprintf(gpLog, "initialize: D3DCompile failed: geometry shader compilation errors\n");
			fprintf(gpLog, "%s\n", (char *)pID3DBlob_Error->GetBufferPointer());
		...
		{
			fprintf(gpLog, "initialize: D3DCompile failed: (geometry shader) there is nothing to print\n");
		}
		fclose(gpLog);
	}
	else
	{
		fopen_s(&gpLog, "Log.txt", "a+");
		fprintf(gpLog, "initialize: D3DCompile succeeded for geometry shader\n");
		fclose(gpLog);
	}
	...
	{
		fopen_s(&gpLog, "Log.txt", "a+");
		fprintf(gpLog, "initialize: CreateGeometryShader failed with error code 0x%lx\n", hr);
		fclose(gpLog);
	}
	else
	{
		fopen_s(&gpLog, "Log.txt", "a+");
		fprintf(gpLog, "initialize: CreateGeometryShader succeeded\n");
		fclose(gpLog);
	}

	// set this geometry shader in the geometry shader stage of pipeline
	...
	d3d11InputElementDescriptor[0].InputSlot = KVD_ATTRIBUTE_POSITION;
	...
	// set into geometry shader stage of pipeline
...
void display(void)
{
	...
	UINT offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(KVD_ATTRIBUTE_POSITION, 1, &gpID3D11Buffer_PositionBuffer, &stride, &offset);

	// set primitive topology in input assembly stage of pipeline
	gpID3D11DeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

	// transformations
	XMMATRIX worldMatrix = XMMatrixTranslation(0.0f, 0.0f, 2.5f);
