...
// to load textures
ID3D11ShaderResourceView *gpID3D11ShaderResourceView_PyramidTexture = NULL;
ID3D11ShaderResourceView *gpID3D11ShaderResourceView_CubeTexture = NULL;
ID3D11SamplerState *gpID3D11SamplerState_Textures = NULL;
...
HRESULT initialize(void)
{
	...
	// tex coords
	const float pyramidVertexTexCoords[] = {
		+0.5, +1.0,
		+1.0, +0.0,
		+0.0, +0.0,
	...
	d3d11BufferDescriptor.ByteWidth = sizeof(float) * _ARRAYSIZE(pyramidVertexTexCoords);
	d3d11BufferDescriptor.BindFlags = D3D11_BIND_VERTEX_BUFFER;

	// populate
	ZeroMemory((void *)&d3d11SubresourceData, sizeof(d3d11SubresourceData));
	d3d11SubresourceData.pSysMem = pyramidVertexTexCoords;

	// create
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDescriptor, &d3d11SubresourceData, &gpID3D11Buffer_TexCoordBuffer_Pyramid);
	if (FAILED(hr))
	{
		fopen_s(&gpLog, "Log.txt", "a+");
		fprintf(gpLog, "initialize: CreateBuffer for Pyramid TexCoord Buffer failed with error code 0x%lx\n", hr);
		fclose(gpLog);
		return hr;
	}
	else
	{
		fopen_s(&gpLog, "Log.txt", "a+");
		fprintf(gpLog, "initialize: CreateBuffer for Pyramid TexCoord Buffer succeeded\n");
		fclose(gpLog);
	}
	...
	// create
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDescriptor, &d3d11SubresourceData, &gpID3D11Buffer_TexCoordBuffer_Cube);
	if (FAILED(hr))
	{
		fopen_s(&gpLog, "Log.txt", "a+");
		fprintf(gpLog, "initialize: CreateBuffer for Cube TexCoord Buffer failed with error code 0x%lx\n", hr);
		fclose(gpLog);
		return hr;
	}
	else
	{
		fopen_s(&gpLog, "Log.txt", "a+");
		fprintf(gpLog, "initialize: CreateBuffer for Cube TexCoord Buffer succeeded\n");
		fclose(gpLog);
	}
	...
	// load textures
	hr = LoadD3DTexture(L"./resources/Stone.bmp", &gpID3D11ShaderResourceView_PyramidTexture);
	if (FAILED(hr))
	{
		fopen_s(&gpLog, "Log.txt", "a+");
		fprintf(gpLog, "initialize: LoadD3DTexture failed for Stone.bmp with error code 0x%lx\n", hr);
		fclose(gpLog);
		return hr;
	}
	else
	{
		fopen_s(&gpLog, "Log.txt", "a+");
		fprintf(gpLog, "initialize: LoadD3DTexture succeeded for Stone.bmp\n");
		fclose(gpLog);
	}

	hr = LoadD3DTexture(L"./resources/Vijay_Kundali.bmp", &gpID3D11ShaderResourceView_CubeTexture);
	if (FAILED(hr))
	{
		fopen_s(&gpLog, "Log.txt", "a+");
	...
	hr = gpID3D11Device->CreateSamplerState(&d3d11SamplerDescriptor, &gpID3D11SamplerState_Textures);
	if (FAILED(hr))
	{
		fopen_s(&gpLog, "Log.txt", "a+");
...
void display(void)
{
	...
	gpID3D11DeviceContext->PSSetShaderResources(0, 1, &gpID3D11ShaderResourceView_PyramidTexture);

	// set texture sampler state in PS (SamplerState in the shader)
	gpID3D11DeviceContext->PSSetSamplers(0, 1, &gpID3D11SamplerState_Textures);

	// draw
	gpID3D11DeviceContext->Draw(12U, 0U);
	...
	// transformations
	worldMatrix = XMMatrixScaling(0.75f, 0.75f, 0.75f) *
	...
	// set texture shader resource view in PS (Texture2D in the shader)
	gpID3D11DeviceContext->PSSetShaderResources(0, 1, &gpID3D11ShaderResourceView_CubeTexture);

	// set texture sampler state in PS (SamplerState in the shader)
	gpID3D11DeviceContext->PSSetSamplers(0, 1, &gpID3D11SamplerState_Textures);

	// draw
	gpID3D11DeviceContext->Draw(36U, 0U);
...
void uninitialize(void)
{
	...
	if (gpID3D11SamplerState_Textures)
	{
		gpID3D11SamplerState_Textures->Release();
		gpID3D11SamplerState_Textures = NULL;
	}
	if (gpID3D11ShaderResourceView_CubeTexture)
	{
		gpID3D11ShaderResourceView_CubeTexture->Release();
		gpID3D11ShaderResourceView_CubeTexture = NULL;
	}
	if (gpID3D11ShaderResourceView_PyramidTexture)
	{
		gpID3D11ShaderResourceView_PyramidTexture->Release();
		gpID3D11ShaderResourceView_PyramidTexture = NULL;
	}
