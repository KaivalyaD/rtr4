...
// texture bytes
BYTE gCheckerboard[CHECKERBOARD_HEIGHT * CHECKERBOARD_WIDTH * 4];
...
HRESULT initialize(void)
{
	...
	// generating the checkerboard texture
	// configure
	D3D11_TEXTURE2D_DESC d3d11Texture2DDescriptor;
	ZeroMemory((void *)&d3d11Texture2DDescriptor, sizeof(d3d11Texture2DDescriptor));
	d3d11Texture2DDescriptor.Width = (UINT)CHECKERBOARD_WIDTH;
	d3d11Texture2DDescriptor.Height = (UINT)CHECKERBOARD_HEIGHT;
	...
	d3d11Texture2DDescriptor.Format = DXGI_FORMAT_R8G8B8A8_UINT;  // 4-bytes per texel
	d3d11Texture2DDescriptor.BindFlags = D3D11_BIND_SHADER_RESOURCE;  // bind as a shader resource, not vertex buffer of DSV
	d3d11Texture2DDescriptor.CPUAccessFlags = 0U;  // default CPU access right
	d3d11Texture2DDescriptor.MiscFlags = 0U;  // no misc.

	// populate
	GenerateCheckerboard();
	ZeroMemory((void *)&d3d11SubresourceData, sizeof(d3d11SubresourceData));
	d3d11SubresourceData.pSysMem = gCheckerboard;

	// allocate a local texture
	ID3D11Texture2D *pID3D11Texture2D = NULL;
	hr = gpID3D11Device->CreateTexture2D(&d3d11Texture2DDescriptor, &d3d11SubresourceData, &pID3D11Texture2D);
	if (FAILED(hr))
	{
		fopen_s(&gpLog, "Log.txt", "a+");
		fprintf(gpLog, "initialize: ID3D11Device::CreateTexture2D failed with error code 0x%lx\n", hr);
		fclose(gpLog);
		return hr;
	}
	else
	{
		fopen_s(&gpLog, "Log.txt", "a+");
		fprintf(gpLog, "initialize: ID3D11Device::CreateTexture2D succeeded\n");
		fclose(gpLog);
	}

	// create a shader resource view from the texture
	hr = gpID3D11Device->CreateShaderResourceView(pID3D11Texture2D, NULL, &gpID3D11ShaderResourceView_Texture);
	if (FAILED(hr))
	{
		fopen_s(&gpLog, "Log.txt", "a+");
		fprintf(gpLog, "initialize: ID3D11Device::CreateShaderResourceView failed with error code 0x%lx\n", hr);
		fclose(gpLog);
		return hr;
	}
	else
	{
		fopen_s(&gpLog, "Log.txt", "a+");
		fprintf(gpLog, "initialize: ID3D11Device::CreateShaderResourceView succeeded\n");
		fclose(gpLog);
	}

	// free the local texture
	if (pID3D11Texture2D)
	{
	...
	d3d11SamplerDescriptor.Filter = D3D11_FILTER_MIN_MAG_MIP_POINT;
...
void display(void)
{
	...
	gpID3D11Device->CreateBuffer(&d3d11BufferDescriptor, &d3d11SubresourceData, &gpID3D11Buffer_PositionBuffer);
	...
	// set tex coord buffer in input assembly stage of pipeline
...
void GenerateCheckerboard(void)
{
	...
	for (int i = 0; i < CHECKERBOARD_HEIGHT; i++)
	{
		for (int j = 0; j < CHECKERBOARD_WIDTH; j++)
		{
			...
			gCheckerboard[(i * CHECKERBOARD_WIDTH) + j + 0] = (BYTE)c;
			gCheckerboard[(i * CHECKERBOARD_WIDTH) + j + 1] = (BYTE)c;
			gCheckerboard[(i * CHECKERBOARD_WIDTH) + j + 2] = (BYTE)c;
			gCheckerboard[(i * CHECKERBOARD_WIDTH) + j + 3] = (BYTE)0xff;
		}
	}
}
