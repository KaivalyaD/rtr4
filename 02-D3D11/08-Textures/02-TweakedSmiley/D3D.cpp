...
HRESULT initialize(void)
{
	...
	// set into vertex & pixel shader stages of pipeline
	...
	// create texture
	hr = LoadD3DTexture(L"./resources/Smiley.bmp", &gpID3D11ShaderResourceView_Texture);
	if (FAILED(hr))
	{
		fopen_s(&gpLog, "Log.txt", "a+");
		fprintf(gpLog, "initialize: LoadD3DTexture failed for Smiley.bmp with error code 0x%lx\n", hr);
		fclose(gpLog);
		return hr;
	}
	else
	{
		fopen_s(&gpLog, "Log.txt", "a+");
		fprintf(gpLog, "initialize: LoadD3DTexture succeeded for Smiley.bmp\n");
		fclose(gpLog);
	}
...
void display(void)
{
	...
	// allocate a constant buffer view and bring into a known state
	CBuffer constantBuffer;
	ZeroMemory((void *)&constantBuffer, sizeof(constantBuffer));

	if (gKeyPresses >= 0 && gKeyPresses < 4)
	{
		float vertexTexCoords[6 * 2];
		switch (gKeyPresses)
		{
		case 0:
			...
			vertexTexCoords[5] = 1.0f;

			vertexTexCoords[6] = 0.0f;
			vertexTexCoords[7] = 1.0f;

			vertexTexCoords[8] = 0.0f;
			vertexTexCoords[9] = 0.5f;

			vertexTexCoords[10] = 0.5f;
			vertexTexCoords[11] = 0.5f;

			constantBuffer.EnableTexture = 1U;
			break;
		...
		case 1:
			...
			vertexTexCoords[2] = 1.0f;
			vertexTexCoords[3] = 0.0f;
			...
			vertexTexCoords[8] = 0.0f;
			vertexTexCoords[9] = 0.0f;

			vertexTexCoords[10] = 1.0f;
			vertexTexCoords[11] = 0.0f;
		...
		case 2:
			...
			vertexTexCoords[2] = 2.0f;
			vertexTexCoords[3] = 0.0f;

			vertexTexCoords[4] = 0.0f;
			vertexTexCoords[5] = 2.0f;

			vertexTexCoords[6] = 0.0f;
			vertexTexCoords[7] = 2.0f;

			vertexTexCoords[8] = 0.0f;
			vertexTexCoords[9] = 0.0f;

			vertexTexCoords[10] = 2.0f;
			vertexTexCoords[11] = 0.0f;
		...
		gpID3D11Device->CreateBuffer(&d3d11BufferDescriptor, &d3d11SubresourceData, &gpID3D11Buffer_TexCoordBuffer);
	...
	{
		constantBuffer.EnableTexture = 0U;
	}
