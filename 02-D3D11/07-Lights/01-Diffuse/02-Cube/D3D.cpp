...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CHAR:
		...
		case 'L':
		case 'l':
			gLight.IsEnabled = (gLight.IsEnabled ? FALSE : TRUE);
			break;
		default:
			break;
		}
...
HRESULT initialize(void)
{
	...
	const char *vertexShaderSourceCode =
		...
		"		output.color = lightLd * lightKd * max(dot(lightDirection, transformedNormal), 0.0f);\n" \
		"	}\n" \
		"	else {\n" \
	...
	gLight.Kd = XMVectorSet(0.5f, 0.5f, 0.5f, 1.0f);
	gLight.Position = XMVectorSet(0.0f, 0.0f, -2.0f, 1.0f);
	gLight.IsEnabled = FALSE;
...
void display(void)
{
	...
	if (gLight.IsEnabled)
	{
		...
		constantBuffer.LightKd = gLight.Kd;
