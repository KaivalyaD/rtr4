...
int main(void)
{
	...
	int *kvd_pi2dArray[COUNT_ROWS], kvd_i, kvd_j;
	...
	for (kvd_i = 0; kvd_i < COUNT_ROWS; kvd_i++)
	{
		...
		{
			printf("malloc(): cannot allocate memory for %d integers in row %d\n\n", COUNT_COLS, kvd_i);
			return 1;
		}
	}
	...
	for (kvd_i = COUNT_ROWS - 1; kvd_i >= 0; kvd_i--)
	{
		if (kvd_pi2dArray[kvd_i])
		{
			...
			printf("freed and cleaned memory block pointed to by kvd_pi2dArray[%d]\n", kvd_i + 1);
		}
	}
